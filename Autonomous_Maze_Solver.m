...
% Saves as a single file and overwrites it every time you run
MazeSolve = VideoWriter('LeftHandSolution.mp4', 'MPEG-4');
MazeSolve.FrameRate = 60; 
open(MazeSolve);
...
map = [1 1 1; 0 0 0; 234/255 170/255 0/255]; 
colormap(map);
img_handle = image(maze+1); 
...
title_handle = title('Left-Hand Rule Navigation (Exploration Phase)');

%% Phase 1: Solve maze using left-hand rule (Exploration)
while ~(current_row == exit_row && current_col == exit_col)
    
    % --- EXIT OVERRIDE (Prevents walking past the exit) ---
    % If the robot is exactly 1 square away, snap to the exit and break!
    if abs(current_row - exit_row) + abs(current_col - exit_col) == 1
        ...
        % Force the final step to draw and record
        temp_maze = maze;
        temp_maze(current_row, current_col) = 2;  
        set(img_handle, 'CData', temp_maze+1);
        drawnow limitrate;
    ...
    valid_forward = (forward_row >= 1 && forward_row <= rows && forward_col >= 1 && forward_col <= cols) && ...
                    (maze(forward_row, forward_col) == 0);
...
%% Phase 2: Visualize the Shortest Path (Optimization)
set(title_handle, 'String', 'Optimized Path'); 
maze_copy = maze;
for k = 1:length(path_row)
    maze_copy(path_row(k), path_col(k)) = 2;  
    
    if mod(k, 2) == 0 || k == length(path_row)
        set(img_handle, 'CData', maze_copy+1);
        drawnow limitrate;
        writeVideo(MazeSolve, getframe(gcf));
    end
end
